...
# Todo : bad function undo_move ... to check.
# Todo : big big problem in restart ...
# TODO : UNDO doesn't handle properly the new data structure

# TODO : PROBLEM WHEN PLAYER HAS AN 1-WIN -> COMPUTER STOP PLAYIG, PROBLEM WITH DICTIONARY SYSTEM ?
...
class Panel(Frame):
    ...
    def __init__(self):
        ...
        # self.history = []
        self.coup = 0
        self.width, self.height = 2, 2  ## TODO ???
        self.cote = 0  ## TODO ???
        self.win = False
        self.coups_possibles = {i: self.n_lig-1 for i in range(self.n_col)}
        self.init_jeu()
    ...
    def init_jeu(self, state=None):
        ...
        self.coups_possibles = self.check_possibles_moves()
        self.trace_grille()
    ...
    def trace_grille(self):
        ...
        self.cote = min(l_max, h_max)  # TODO; ? Is there a way to avoid this step, and update self.cote in live ?
    ...
    def click(self, event):
        ...
        if self.player == 1:  # because the human is Yellow
    ...
    def play(self, col, lig=0):
        if 0 <= lig < self.n_lig and 0 <= col < self.n_col:
            ...
            # the side of a case would be the smallest of the two dimensions :
            self.cote = min(l_max, h_max)  # TODO; ? Is there a way to avoid this step, and update self.cote in live ?
            # TODO : following code useless ??? (see rescale function)
            wide, high = self.cote * self.n_col, self.cote * self.n_lig
            self.can.configure(width=wide, height=high)
            n = 0
            x1 = self.cote * col + 3
            ...
            if 1 <= n <= self.n_lig:
                ...
                if n == 1:
                    del self.coups_possibles[col]
                else:
                    self.coups_possibles[col] -= 1
    ...
    def play_game(self, col, lig):  # TODO : UPDATE new data structure for coups_possibles. ?
    ...
    def brut_force(self, proof):  # TODO : UPDATE new data structure for coups_possibles. ?
        save = self.state.copy()
        list_weight = ListWeight()

        if proof == 0:
            d = self.coups_possibles.copy()
            for i in d:
                j = d[i]
                self.state[j][i] = 0  # Computer is 0 (Red) here
