...
Source : https://www.techwithtim.net/tutorials/game-development-with-python/tetris-pygame/
...
class Piece(object):
    ...
    def __init__(self, column, row, shape):
        ...
        self.y = row
        self.shape = shape
        self.color = self._shape_colors[self._shapes.index(shape)]
        self.rotation = 0  # number from 0-3

    @classmethod
    def get_shape(cls):
        return Piece(5, 0, random.choice(cls._shapes))


def create_grid(locked_positions=None):
    if locked_positions is None:
        locked_positions = {}
    grid_created = [[(0, 0, 0) for _ in range(10)] for _ in range(20)]

    for i in range(len(grid_created)):
        for j in range(len(grid_created[i])):
...
def main():
    ...
    change_piece = False
    run = True
    current_piece = Piece.get_shape()
    next_piece = Piece.get_shape()
